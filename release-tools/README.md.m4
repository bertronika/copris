...
You, as a user, can invoke a custom command from within the text. Specify the `-c` argument when running COPRIS and begin your text with one of the following commands: `$ENABLE_COMMANDS`, `$ENABLE_CMD` or `$CMD`. You may then invoke commands in the text file with omitting their `C_` prefix and prepending a dollar sign symbol to them. I.e., if your command is `C_SERIF`, `$SERIF` is used in text to invoke it.
