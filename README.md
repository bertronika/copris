...
Run as a simple server on port 8080, perform no text conversion, output received data to the
...
Serve on port 8080 as a daemon (do not exit after first connection), translate characters using
the `slovene.ini` translation file, limit any incoming text to maximum 100 characters and print
...
character that isn't present in the ASCII character set. Output formatted text to a USB interface
