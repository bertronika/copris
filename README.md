...
No indentation is permitted before writing out left-hand keys. White space at beginning of
lines will result in (sometimes) confusing error messages - unfortunately a limitation of the
parsing library we use.

However, any amount of spaces (or tabulators) are allowed after that. Keep in mind not to
prepend zeroes to decimal numbers for alignment, as they'll be interpreted as octal numbers.
